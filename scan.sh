...
optimizer="Treewidth(alg=MF())" overwrite=true problem_list="$PROBLEM" make run1
optimizer="Treewidth(alg=MMD())" overwrite=true problem_list="$PROBLEM" make run1
optimizer="Treewidth(alg=AMF())" overwrite=true problem_list="$PROBLEM" make run1
...
# optimizer="HyperND(; dis = METISND(), score=$TCSCORE, width=120, imbalances=130:1:130)" problem_list="$PROBLEM" make run1
# optimizer="HyperND(; dis = KaHyParND(), score=$TCSCORE, width=120, imbalances=130:1:130)" problem_list="$PROBLEM" make run1
...
# optimizer="HyperND(; dis=METISND(), score=$SCSCORE, width=120, imbalances=130:1:130)" problem_list="$PROBLEM" make run1
# optimizer="HyperND(; dis=KaHyParND(), score=$SCSCORE, width=120, imbalances=130:1:130)" problem_list="$PROBLEM" make run1
...
for niters in {0..10}; do 
    ...
    optimizer="GreedyMethod(α=$alpha)" overwrite=true problem_list="$PROBLEM" make run1
done
